...
export function ChatTab({
  ...
  }, [clientId, project.id, activeConvId]);
  ...
  return (
    <div className="flex flex-1 h-full bg-zinc-50 dark:bg-zinc-950 relative">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        {/* Mobile top bar */}
        <div className="md:hidden shrink-0 flex items-center justify-between px-4 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
      ...
      {/* ── Desktop sidebar ── */}
      <aside
        className="hidden md:flex shrink-0 border-l border-zinc-200 dark:border-zinc-800 overflow-y-auto flex-col bg-white dark:bg-zinc-950"
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-600 mt-1 max-w-[240px]">
        {noConversation
          ? "Appuie sur « Historique » puis « + Nouvelle conversation » pour commencer"
...
function ChatMessage({ msg, clientColor }: { msg: Message; clientColor: string }) {
  ...
  return (
    <div className={`flex gap-2 sm:gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      ...
      <div
        className={`max-w-[85%] sm:max-w-[68%] rounded-xl px-3 sm:px-4 py-3 text-sm leading-relaxed ${
          isUser
