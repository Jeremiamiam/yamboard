...
const STAGE_KEYS = ["devis", "acompte", "avancement", "solde"] as const;
...
function ProductDrawerContent({
  ...
  // Read live product from store (updates in real time via optimistic)
  ...
  const definedStages = STAGE_KEYS.filter((k) => liveProduct[k] != null);
  const missingStages = STAGE_KEYS.filter((k) => liveProduct[k] == null);
  ...
  return (
    <div className="fixed top-0 right-0 bottom-0 z-50 w-full sm:w-[480px] flex flex-col bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden">
        ...
        {editingHeader ? (
          <div className="flex-1 space-y-2 mr-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={saveHeader} className="ml-2 text-xs font-medium text-emerald-600 hover:text-emerald-500">
                OK
              </button>
              <button onClick={() => setEditingHeader(false)} className="text-xs text-zinc-400 hover:text-zinc-600">
                ✕
              </button>
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-3">
        ...
        {STAGE_KEYS.filter((k) => liveProduct[k] != null).map((key) => (
          <StageRow
            key={key}
            stageKey={key}
...
function StageRow({
  ...
  const statusConfig = {
    pending: { dot: "bg-zinc-400", label: "En attente", text: "text-zinc-500" },
    sent: { dot: "bg-yellow-500", label: "Envoyé", text: "text-yellow-500" },
    paid: { dot: "bg-emerald-500", label: "Payé ✓", text: "text-emerald-500" },
  }[stage.status];

  return (
    <div className="rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-3 space-y-3">
      {/* Stage name + status toggle */}
      ...
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          onBlur={() => {
            if (editDate !== (stage.date ?? "")) save({ date: editDate || undefined });
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              save({ date: editDate || undefined });
              (e.target as HTMLInputElement).blur();
            }
          }}
      ...
      {stage.status === "paid" && stage.amount && (
        <div
          ...
          style={{ background: clientColor ?? "#10b981" }}
        />
      )}
    </div>
  );
